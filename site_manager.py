...
""")
    ...
    def create_nginx_config(self, domain, php_version, ssl_enabled=False, php_settings=None):
        ...
        # Build config
        if ssl_enabled:
            # SSL-enabled configuration with separate HTTP and HTTPS blocks
...
"""
    ...
    def create_panel_nginx_config(self, panel_domain=None, ssl_enabled=False):
        """Create or update Nginx configuration for the panel itself
        ...
        if ssl_enabled and panel_domain:
            # SSL-enabled configuration
            config = f"""# Nginx configuration for Lala Panel
# Generated by Lala Panel
...
# HTTPS server - main panel access
server {{
    listen 443 ssl http2;
    listen [::]:443 ssl http2;
    server_name {panel_domain};
...
"""
            ...
            # Non-SSL configuration (default or no domain set)
            config = f"""# Nginx configuration for Lala Panel
# Generated by Lala Panel
...
"""
        ...
        enabled_path = os.path.join(self.nginx_enabled, 'lalapanel')
        if not os.path.exists(enabled_path):
    ...
    def create_default_server_config(self):
        """Create default server config to reject unknown HTTPS domains
        
        Note: The HTTP default_server is handled by the panel nginx configuration
        """
...
# HTTP requests are handled by the panel configuration
