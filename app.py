...
def get_webauthn_rp_id():
    ...
    # Use the request host or fall back to localhost
    if request:
...
def passkey_register_begin():
    ...
    try:
        ...
        existing_creds = db.get_passkey_credentials_for_user(user_id)
        exclude_credentials = [
            PublicKeyCredentialDescriptor(id=base64.urlsafe_b64decode(cred['credential_id']))
            for cred in existing_creds
        ]
        
        # Generate registration options supporting all authenticator types
        ...
        session['passkey_challenge'] = base64.urlsafe_b64encode(registration_options.challenge).decode('utf-8')
        ...
        app.logger.error(f"Passkey registration begin error: {e}")
...
def passkey_register_complete():
    ...
    try:
        ...
        # Get device type from authenticator data
        ...
        transports = None
        if 'transports' in data.get('response', {}):
            transports = ','.join(data['response']['transports'])
        
        # Get credential name from request
        credential_name = data.get('name', f'Passkey {datetime.now().strftime("%Y-%m-%d %H:%M")}')
        
        db.create_passkey_credential(
            user_id=user_id,
        ...
        audit_log('passkey_registered', f'User registered a new passkey: {credential_name}')
        
        return jsonify({'success': True, 'message': 'Passkey registered successfully'})
        
    except Exception as e:
        app.logger.error(f"Passkey registration complete error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 400
...
def passkey_login_begin():
    ...
    try:
        ...
        credentials = db.get_passkey_credentials_for_user(user_id)
        if not credentials:
            return jsonify({'success': False, 'error': 'No passkeys registered'}), 400
        
        # Create list of allowed credentials
        allow_credentials = [
            PublicKeyCredentialDescriptor(id=base64.urlsafe_b64decode(cred['credential_id']))
        ...
        session['passkey_auth_challenge'] = base64.urlsafe_b64encode(authentication_options.challenge).decode('utf-8')
        ...
        app.logger.error(f"Passkey login begin error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 400
...
def passkey_login_complete():
    ...
    try:
        ...
        # Get challenge and user_id from session
        challenge = session.get('passkey_auth_challenge')
        user_id = session.get('passkey_auth_user_id')
        
        if not challenge or not user_id:
            return jsonify({'success': False, 'error': 'No authentication session found'}), 400
        
        challenge_bytes = base64.urlsafe_b64decode(challenge)
        
        # Get credential from database
        credential_id = base64.urlsafe_b64encode(base64.urlsafe_b64decode(data['id'])).decode('utf-8')
        credential = db.get_passkey_credential(credential_id)
        ...
        # Log in user
        user_data = db.get_user(None)  # Need to get user by ID
        with db.get_connection() as conn:
            cursor = conn.cursor()
        ...
        if user_data:
            ...
            audit_log('passkey_login', f'User logged in with passkey: {credential["name"]}', user_id)
        ...
        app.logger.error(f"Passkey login complete error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 400
...
def passkey_list():
    ...
    try:
        credentials = db.get_passkey_credentials_for_user(current_user.id)
        ...
        app.logger.error(f"Passkey list error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 400
...
def passkey_delete():
    ...
    try:
        ...
        if not credential or credential['user_id'] != current_user.id:
            return jsonify({'success': False, 'error': 'Invalid credential'}), 403
        
        db.delete_passkey_credential(credential_id)
        
        audit_log('passkey_deleted', f'User deleted passkey: {credential["name"]}')
        
        return jsonify({'success': True, 'message': 'Passkey deleted successfully'})
        
    except Exception as e:
        app.logger.error(f"Passkey delete error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/passkey/rename', methods=['POST'])
@login_required
@limiter.limit("10 per minute")
def passkey_rename():
    ...
    try:
        ...
        if not credential_id or not new_name:
            return jsonify({'success': False, 'error': 'Credential ID and name required'}), 400
        ...
        db.update_passkey_name(credential_id, new_name)
        
        return jsonify({'success': True, 'message': 'Passkey renamed successfully'})
        
    except Exception as e:
        app.logger.error(f"Passkey rename error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 400
...
def passkeys():
    ...
    init_components()
    credentials = db.get_passkey_credentials_for_user(current_user.id)
    return render_template('passkeys.html', credentials=credentials)
